    ...
    <div class="bg-gradient-to-r from-indigo-600 to-blue-500 rounded-xl shadow-lg p-6">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xl sm:text-2xl font-bold text-white tracking-wide">DATA GURU</h3>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 gap-4 md:hidden">
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 shadow-sm">
                <div class="flex justify-between items-start gap-3">
                    <div class="space-y-1">
                        <p class="font-bold text-gray-800 text-lg">{{ $teacher['name'] ?? 'N/A' }}</p>
                ...
                <div class="mt-4 pt-4 border-t border-gray-200 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="font-medium text-gray-700">{{ $teacher['phone'] }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">Jabatan</p>
                        <p class="font-medium text-gray-700">{{ $teacher['role_position']['name'] ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
            @empty
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 font-medium">{{
                            $teacher['name'] ?? 'N/A' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $teacher['nip'] }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $teacher['phone'] }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{
                            $teacher['role_position']['name'] ?? 'N/A' }}</td>
        ...
        <div @click.away="$wire.closeModal()"
            ...
            <form wire:submit.prevent="store">
                ...
                <div class="p-6 space-y-4">
                    {{-- CATATAN: Untuk User & Jabatan, idealnya menggunakan <select> dropdown --}}
                        <div>
                            <label for="user_id" class="block text-sm font-medium text-gray-700">User ID</label>
                            <input wire:model="user_id" type="number" id="user_id" placeholder="Masukkan ID User"
                            ...
                            @error('user_id') <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            <label for="nip" class="block text-sm font-medium text-gray-700">NIP</label>
                            <input wire:model="nip" type="text" id="nip" placeholder="Contoh: 199001012020121001"
                                class="mt-1 px-3 py-2 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        <div>
                            <label for="role_position_id" class="block text-sm font-medium text-gray-700">Jabatan
                                ID</label>
                            <input wire:model="role_position_id" type="number" id="role_position_id"
                                placeholder="Masukkan ID Jabatan"
                                class="mt-1 px-3 py-2 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            @error('role_position_id') <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                </div>
                <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3">
                    ...
                    <button type="submit"
                        ...
                        {{ $teacherId ? 'Simpan Perubahan' : 'Simpan' }}
                    </button>
                </div>
            </form>
        </div>
