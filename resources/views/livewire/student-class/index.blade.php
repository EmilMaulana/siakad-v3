<div>
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                <div class="bg-white w-12 h-12 flex items-center justify-center rounded-full shadow-md shrink-0">
                    <i class="fas fa-users fa-fw text-indigo-600 text-lg"></i>
                </div>
                <div>
            ...
            <button wire:click="openModal()"
                ...
                <span>Tambah Penempatan Siswa</span>
            </button>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-4 sm:p-6 mt-6 relative">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-4">
            <input type="text" wire:model.live="search" placeholder="Cari Penempatan Siswa..."
                class="px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 w-full">
        </div>

        {{-- Mobile Card View --}}
        <div class="grid grid-cols-1 gap-4 md:hidden">
            @forelse($studentClasses as $item)
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 shadow-sm">
                <div class="flex justify-between items-start gap-3">
                    <div class="space-y-1">
                        <p class="font-bold text-gray-800 text-lg">{{ $item['student']['name'] ?? 'N/A' }}</p>
                        <p class="text-sm text-gray-600">Kelas: {{ $item['class']['name'] ?? 'N/A' }}</p>
                    </div>
                ...
                <div class="mt-4 pt-4 border-t border-gray-200 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="font-medium text-gray-700">{{ $item['student']['nis'] ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">Kode Kelas</p>
                        <p class="font-medium text-gray-700">{{ $item['class']['code'] ?? '-' }}</p>
                    </div>
                </div>
            </div>
            @empty
            <div class="col-span-1 py-12 text-center text-gray-500">
                <div class="flex flex-col items-center">
                    ...
                    <span>Tidak ada data penempatan siswa ditemukan.</span>
                </div>
            </div>
            @endforelse
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 font-medium">{{
                            $item['student']['name'] ?? 'N/A' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $item['student']['nis'] ?? '-'
                            }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $item['class']['name'] ?? '-'
                            }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $item['class']['code'] ?? '-'
                            }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            {{$item['class']['academic_year']['year']?? '-'}}</td>
                        </td>
            ...
            <form wire:submit.prevent="store">
                ...
                <div class="bg-gradient-to-r from-indigo-600 to-blue-500 px-6 py-4">
                    <h3 class="text-lg font-bold text-white">
                        {{ $studentClassId ? 'Edit Penempatan Siswa' : 'Tambah Penempatan Siswa' }}
                    </h3>
                </div>
                ...
                <div class="p-6 space-y-4">
                    ...
                    const studentChoices = new Choices($refs.selectStudent, {
                        searchEnabled: true,
                        itemSelectText: "",
                        allowHTML: true,
                    });
                    ...
                    studentChoices.setChoiceByValue(String($wire.student_id));

                    $watch("$wire.student_id", (newValue) => {
                        studentChoices.setChoiceByValue(String(newValue));
                    });

                    $refs.selectStudent.addEventListener("change", (event) => {
                        $wire.set("student_id", event.target.value);
                    });
                '>
                        <label for="student_id" class="block text-sm font-medium text-gray-700">Siswa</label>
                        <select x-ref="selectStudent"></select>
                    </div>
                    @error('student_id')
                ...
                <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 mt-20">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg 
                        ...
                        {{ $studentClassId ? 'Simpan Perubahan' : 'Simpan' }}
                    </button>
                </div>
            </form>
        </div>
